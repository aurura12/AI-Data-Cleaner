...
TRANSLATIONS = {
    '中文': {
        ...
        **2. 特征工程**
        基于数据分布构建了高阶特征：
        *   **核心特征合成**：根据列语义关系自动合成新的衍生特征（如计算总和、比率等）。
        *   **离散化与分箱**：对连续特征进行分箱，捕捉非线性关系。
        *   **异常形态检测**：引入 Z-score 标准分算法，自动识别偏离统计均值的异常样本。

        **3. 多维溯源**
        对 **ID 列** 进行结构化解构，自动提取 **源产地、批次ID、空间坐标** 以及 **时间序列**。将单一的ID字符串转化为可用于时空相关性分析的独立维度。

        **4. 标签标准化与降噪**
        *   **目标构建**：将离散的目标值（如分类标签）映射为标准的二分类标签 (Pass/Fail)，为机器学习模型提供明确的监督信号。
        *   **特征选择**：根据业务相关性，自动剔除低方差或全空列，有效降低数据噪声，提升后续分析的信噪比。
        """,
        ...
        *   “分析一下温度和良率之间有没有关系”
        ...
        'ai_placeholder': "请输入分析需求 (例如: 画图展示关键特征分布)",
        ...
        "ai_placeholder_input": "例如：分析一下关键特征的分布情况",
...
TRANSLATIONS['中文'].update({
    'pdf_title': "生产工艺质量分析报告",
    'pdf_date': "生成日期: ",
    'pdf_sec_1': "1. 核心指标概览",
    'pdf_yield': "整体良品率",
    'pdf_open_fail': "不合格率",
    'pdf_severe_fail': "严重缺陷率",
    'pdf_baseline': "基准线",
    'pdf_count': "数量: ",
    ...
    'html_sec_3': "第三部分：总结与优化建议",
    'html_no_mining': "暂无深度挖掘结果。",
    'html_no_stats': "暂无描述性统计报告。",
    'html_no_sug': "暂无最终建议。",
    'dl_title': "📥 完整报告导出",
    'dl_intro': "该模块将整合【描述性统计分析】、【深度挖掘】以及【优化建议】，生成一份包含所有图表和智能分析的可离线阅读报告。",
...
TRANSLATIONS['English'].update({
    ...
    'pdf_title': "Data Analysis Panorama Report",
    'pdf_date': "Generated At: ",
    'pdf_sec_1': "1. KPI Overview",
    'pdf_yield': "Overall Yield",
    'pdf_open_fail': "Fail Rate",
    'pdf_severe_fail': "Defect Rate",
    ...
    'pdf_suggestions': "<b>💡 Suggestions:</b>",
    ...
    'dl_intro': "This module integrates descriptive stats, deep mining, and optimization suggestions into an offline-ready report with all charts and AI analysis.",
    ...
    'dl_item_sug': "💡 Optimization Suggestions",
